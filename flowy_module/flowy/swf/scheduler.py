...
try:
    ...
    from boto_exceptions import SWFResponseError
    from layer1_decisions import Layer1Decisions
...
class DecisionScheduler(object):
    ...
    def remote_subworkflow(self, task_id, input, result_deserializer,
        ...
        if result is None:
            name, version = task_id
            rand_id = ''.join(random.choice(string.ascii_uppercase +
                                            string.digits) for x in range(10))
            subworkflow_id = '%s-%s' % (rand_id, self._call_id)
    ...
    def _search_result(self, retry, result_deserializer, error_handling):
        for _call_id in range(self._call_id, self._call_id + retry + 1):
