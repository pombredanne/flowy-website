...
class OptionsScheduler(object):
    ...
    def options(self,
        ...
        new_dict = dict(self._activity_options_stack[-1])
        for k in a_options:
            new_dict[k] = a_options[k]
        self._activity_options_stack.append(new_dict)
        new_dict = dict(self._subworkflow_options_stack[-1])
        for k in s_options:
            new_dict[k] = s_options[k]
        self._subworkflow_options_stack.append(new_dict)
        yield 0
